...
select avg(presupuesto) as total_presupuesto from departamento;

-- 3.Calcula el valor mínimo del presupuesto de todos los departamentos.
select min(presupuesto) as valor_minimo from departamento;

-- 4. Calcula el nombre del departamento y el presupuesto que tiene asignado,
-- del departamento con menor presupuesto.
select nombre from departamento order by presupuesto asc limit 1;

-- 5.Calcula el valor máximo del presupuesto de todos los departamentos.
select max(presupuesto) as valor_minimo from departamento;

-- 6.Calcula el nombre del departamento y el presupuesto que tiene asignado,
-- del departamento con mayor presupuesto.
select nombre from departamento order by presupuesto desc limit 1;
...
select count(*) as total_empleados from empleado where apellido2 is not null;
...
select d.nombre as nombre_departamento, count(e.id) as cantidad_empleado from departamento d inner join empleado e on d.id=e.id_departamento group by 1;
...
select d.nombre as nombre_departamento, count(e.id) as cantidad_empleado from departamento d left join empleado e on d.id=e.id_departamento group by 1;

...
select distinct d.nombre as Departamento, a.nombre as asignaturas_asignadas from profesor p inner join departamento d on p.id_departamento= d.id inner join asignatura a on p.id=a.id_profesor left join alumno_se_matricula_asignatura amt on a.id=amt.id_asignatura;
...
select distinct * from profesor where id_departamento is null;

-- 10. Devuelve un listado con los departamentos que no tienen profesores asociados.
select * from departamento d left join profesor p on p.id_departamento=d.id where p.id_departamento is null;

-- 11. Devuelve un listado con los profesores que no imparten ninguna asignatura.
 select * from profesor p left join asignatura a on a.id_profesor=p.id where a.id_profesor is null;
 -- select * from asignatura;
 
-- 12.Devuelve un listado con las asignaturas que no tienen un profesor asignado.
select * from asignatura where id_profesor is null;

-- 13. Devuelve un listado con todos los departamentos que tienen alguna asignatura que no se haya impartido en ningún curso escolar. 
-- El resultado debe mostrar el nombre del departamento y el nombre de la asignatura que no se haya impartido nunca.

select d.nombre as Departamento, a.nombre as asignaturas_asignadas from profesor p inner join departamento d on p.id_departamento= d.id inner join asignatura a on a.id_profesor =p.id left join alumno_se_matricula_asignatura amt on a.id=amt.id_asignatura;

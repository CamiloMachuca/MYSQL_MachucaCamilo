...
-- 3.  muestre los clientes con sus respectivos alquileres y el empleado que lo atendio ademas del vehiculo que alquilo.
...
-- 4.  muestre los clientes con sus respectivos alquileres y el empleado que lo atendio ademas del vehiculo que alquilo teniendo en cuenta que el vehiculo sea de color negro y tenga cuatro puertas.
select a.id_alquiler, c.nombre as nombre_cliente, e.nombre as nombre_empleado, v.tipo as tipo_Vehiculo from alquileres a 
inner join clientes c on a.id_cliente= c.id_cliente 
inner join empleados e on e.id_empleado=a.id_cliente 
inner join vehiculos v on v.id_vehiculo=a.id_cliente where v.color='negro' and v.puertas=4 order by 1; 

-- 5. muestre todas las sucursales de las ciudades Bogota, cucuta con sus clientes y el empleado que atendio a ese cliente ademas de que tipo de vehiculo alquilaron los clientes
...
-- 6. muestre los vehiculos que tengan un descuento mayor a 20% y que ese descuento dure mas de dos meses activo
...
-- 7. muestre los clientes con sus respectivos alquileres y el empleado que lo atendio ademas del vehiculo que alquilo y el descuento con el que dispone.
...
inner join descuento d on d.id_descuento= v.id_vehiculo
 ...
 -- 8. muestreme el nombre del cliente que tiene un descuento ademas de mostrar el valor_cotizado con el descuento ya aplicado
 delimiter //
create function Descuento_aplicado(porcentaje_descuento decimal(10,2), valor_cotizado decimal(10,2))
returns decimal(10,2)
deterministic
begin
	declare valor_con_descuento int;
    set valor_con_descuento= valor_cotizado -valor_cotizado*(porcentaje_descuento/ 100);
...
-- 9. calcular y mostrar la direfencia del valor cotizado y el valor pagado mostrar el valor cotizado y el valor pagado 
 delimiter //
...
-- 10.mostrar el nombre del cliente junto con la direfencia del valor cotizado y el valor pagado ademas mostrar el valor cotizado y el valor pagado 
...
-- 11. mostrar la cantidad de dias que pasaron desde el dia que alquilaron un vehiculo hasta el dia que lo entregaron 
...
-- 12.muestre los vehiculos que tengan un descuento mayor a 20% y que ese descuento dure mas de 10 dias activo
...
-- 13. muestreme los vehiculos que tenga un sunroof

select * from vehiculos where sunroof = 'no';

-- 14. muestrame los datos de los tres vehiculos que tengan el descuento mas alto
...
inner join descuento d on d.id_descuento= v.id_vehiculo
 ...
 -- 22. muestreme  los datos de los empleados con el nombre de los empleados en minusculas y los apellidos en mayusculas

 select lower( nombre), upper(apellido1) as apellido1, upper(apellido2) as apellido2, direccion, correo_electronico from clientes;
 
 -- 23. muestre los datos de la tabla descuento junto con los dias que van a durar cada descuento
 -- select * from alquileres;
 delimiter //
create function Dias_descuento(inicio_descuento date, fin_descuento date)
returns int
deterministic
begin
	declare total int;
    set total= timestampdiff(day,inicio_descuento,fin_descuento);
	return total;
end // 
delimiter ;
-- drop function Dias_descuento;
select id_descuento, id_vehiculo, porcentaje_descuento, inicio_descuento, fin_descuento, Dias_descuento(d.inicio_descuento ,d.fin_descuento) as dias_descuento from descuento d;


-- 24. muestre el id_del cliente, el id del vehiculo y los dias de retraso que tuvo cada cliente
-- select * from alquileres;
 delimiter //
create function Dias_retraso(fecha_esperada_llegada date, fecha_llegada date)
returns int
deterministic
begin
	declare total int;
    set total= timestampdiff(day,fecha_esperada_llegada,fecha_llegada);
	return total;
end // 
delimiter ;

select id_cliente, id_vehiculo, Dias_retraso(fecha_esperada_llegada, fecha_llegada) as dias_retrasos from alquileres;

-- 25.muestre los clientes con sus respectivos alquileres y el empleado que lo atendio ademas del vehiculo que alquilo, 
-- el descuento con el que dispone y los dias que tuvo de retraso junto con los dias que alquilo el vehiculo de los primeros 10 clientes.
select a.id_alquiler,d.porcentaje_descuento, c.nombre as nombre_cliente, e.nombre as nombre_empleado, v.tipo as 
tipo_Vehiculo, v.id_vehiculo, Dias_retraso(a.fecha_esperada_llegada, a.fecha_llegada) as dias_retrasos, Dias_alquiler_vehiculo(a.fecha_salida ,a.fecha_llegada) as dias_en_alquiler 
from alquileres a 
inner join clientes c on a.id_cliente= c.id_cliente 
inner join empleados e on e.id_empleado=a.id_cliente 
inner join vehiculos v on v.id_vehiculo=a.id_cliente 
inner join descuento d on d.id_descuento= v.id_vehiculo
 ...
 -- 26.  muestre el id_del cliente, el id del vehiculo y los dias de retraso que tuvo cada cliente ordenados de mayor a menor por el id del cliente
 select id_cliente, id_vehiculo, Dias_retraso(fecha_esperada_llegada, fecha_llegada) as dias_retrasos from alquileres order by id_cliente desc;
 
 -- 27.  muestre los datos de la tabla descuento junto con los dias que van a durar cada descuento ademas de mostrar los dias de retraso
 select d.id_descuento, d.id_vehiculo, d.porcentaje_descuento, d.inicio_descuento, d.fin_descuento, Dias_descuento(d.inicio_descuento ,d.fin_descuento) as dias_descuento, 
 Dias_retraso(a.fecha_esperada_llegada, a.fecha_llegada) as dias_retrasos from descuento d inner join alquileres a;

 -- 28. mostrar la cantidad de dias que pasaron desde el dia que alquilaron un vehiculo hasta el dia que lo entregaron ordenados de forma alfabeticamente por el nombre del cliente
select a.id_alquiler,c.nombre, Dias_alquiler_vehiculo(a.fecha_salida ,a.fecha_llegada) as dias_en_alquiler from alquileres a inner join clientes c on c.id_cliente= a.id_cliente order by c.nombre asc;
 
 -- 29. muestre los datos de la tabla descuento junto con los dias que van a durar cada descuento ademas de mostrar los dias de retraso 
 -- ademas de mostrar la diferencia del valor cotizado y el valor pagado.
 select d.id_descuento, d.id_vehiculo, d.porcentaje_descuento, d.inicio_descuento, d.fin_descuento, Dias_descuento(d.inicio_descuento ,d.fin_descuento) as dias_descuento, 
 Dias_retraso(a.fecha_esperada_llegada, a.fecha_llegada) as dias_retrasos, diferencia_valor_cotizado_pagado(a.valor_cotizado,a.valor_pagado) from descuento d inner join alquileres a;
 
 -- 30. mostrar la cantidad de dias que pasaron desde el dia que alquilaron un vehiculo hasta el dia que lo entregaron ordenados de forma alfabeticamente por el nombre del cliente incleye toda la tabla de cliente.
select a.id_alquiler,c.nombre, Dias_alquiler_vehiculo(a.fecha_salida ,a.fecha_llegada) as dias_en_alquiler from alquileres a right join clientes c on c.id_cliente= a.id_cliente order by c.nombre asc;
...
delimiter //
create procedure insertarEmpleados(in ciudad_residencia Varchar(50), cedula varchar(50), nombre varchar(50), apellido1 varchar(50), apellido2 varchar(50), direccion varchar(50), celular varchar(50), id_sucursal int, correo_electronico varchar(50))
begin
insert into empleados(ciudad_residencia, cedula, nombre, apellido1, apellido2, direccion, celular, id_sucursal, correo_electronico ) values(ciudad_residencia, cedula, nombre, apellido1, apellido2, direccion, celular, id_sucursal, correo_electronico);

end 
// delimiter ;

call insertarEmpleados("cúcuta", "1090397666", "camilo", "ortega", "rincon", "KDX21-69-20", "109039766", 2, "camilo1090@gmail.com");

-- ########################################################
-- ########################################################
-- 3. proceso para añadir un nuevo cliente
delimiter //
create procedure insertarClientes(in cedula varchar(50), nombre varchar(50), apellido1 varchar(50), apellido2 varchar(50), direccion varchar(50), correo_electronico varchar(50),celular varchar(50), ciudad_residencia varchar(50))
begin
insert into clientes(cedula, nombre, apellido1, apellido2, direccion, correo_electronico, celular, ciudad_residencia  ) values(cedula, nombre, apellido1, apellido2, direccion, correo_electronico, celular, ciudad_residencia );

end 
// delimiter ;
call insertarClientes("1090397666", "camilo", "ortega", "rincon", "KDX21-69-20", "camilo1090@gmail.com", "1335567565", "medellin" );
-- ########################################################
select * from clientes;
...
-- ########################################################
-- 4.proceso para cambiar el valor cotizado de algun alquiler.
delimiter //
create procedure cambiar_valorCotizado(in id int, valor_cotizado decimal(10,2))
begin
	update alquileres set valor_cotizado= valor_cotizado where id_alquiler= id;
end 
// delimiter ;
-- drop procedure cambiar_valorCotizado;
call cambiar_valorCotizado (1,"300");
select * from alquileres;


-- ########################################################
-- 5. proceso para aplicar el descuento al valor cotizado de un alquiler que elija el usuario.
delimiter //
create procedure cambiar_aplicarDescuento(in id int, valor_final decimal(10,2))
begin
	update alquileres set valor_cotizado= valor_final where id_alquiler= id;
end 
// delimiter ;
-- drop procedure cambiar_valorCotizado;
call cambiar_aplicarDescuento (2,(select Descuento_aplicado(d.porcentaje_descuento, a.valor_cotizado ) from descuento d 
inner join alquileres a on a.id_vehiculo=d.id_vehiculo 
inner join clientes c on c.id_cliente=a.id_cliente where a.id_alquiler= 2 limit 1
));
...
delimiter //
create procedure añadir_carro (in sunroof varchar(50), capacidad varchar(50), tipo varchar(50), placa varchar(50), referencia varchar(50), modelo varchar(50), puertas int, color varchar(50), motor varchar(50))
begin 
insert into vehiculos(sunroof, capacidad, tipo, placa, referencia, modelo, puertas, color, motor) values (sunroof, capacidad, tipo, placa, referencia, modelo, puertas, color, motor);
end
// delimiter ;
...
begin 
insert into alquileres (fecha_llegada, id_cliente, id_empleado, id_vehiculo, id_sucursal, fecha_salida, 
fecha_esperada_llegada, valor_alquiler_dia, valor_alquiler_semana, valor_cotizado, valor_pagado, id_descuento) values (fecha_llegada, id_cliente, id_empleado, id_vehiculo, id_sucursal, fecha_salida, 
fecha_esperada_llegada, valor_alquiler_dia, valor_alquiler_semana, valor_cotizado, valor_pagado, id_descuento);
end 
// delimiter ;
